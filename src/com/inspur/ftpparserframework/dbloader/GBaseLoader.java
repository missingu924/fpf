 ...
 * 
 */
public class GBaseLoader implements IDbLoader
{
			...
			throws Exception
	{
		...
		try
		{
			...
			// 3shell
			// gbloader -ugbase -pgbase20110531 -T10 -l${LOGFILE_DIR}/${filename}.log
			// -C${BADFILE_DIR}/${filename}.errdata ${CTLFILE_DIR}/${filename}.ctl
			...
			String shell = "/opt/gbase8a/server/bin/gbloader -u" + userId + " -p" + userPass + " -P" + gbPort + " -h "
					+ gbIp + " -T10" + " -l" + logFile.getCanonicalPath() + " -C" + badFile.getCanonicalPath() + " "
	...
	private void parseLog(File logFile, StringBuffer succRows, StringBuffer errorRows, StringBuffer errorLog)
	{
		...
		try
		{
			...
			{
				int succRowsIndex = line.indexOf("Load Records:");// 英文日志
				if (succRowsIndex >= 0)
				{
					succRows.append(line.substring(succRowsIndex + "Load Records:".length()).trim());
				}

				int errorRowsIndex = line.indexOf("kipped Records:");
				if (errorRowsIndex >= 0)
				{
					errorRows.append(line.substring(errorRowsIndex + "kipped Records:".length()).trim());
				}

				if (line.indexOf("data error") >= 0 && errorLog.length() == 0)// 只取一行错误信息，避免信息过多日志复杂
				{
					errorLog.append(line.substring(line.indexOf("data error")).trim());
				}
			}
		} catch (Exception e)
		{
